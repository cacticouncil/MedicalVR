...
public class _TGvrReticle : MonoBehaviour, IGvrGazePointer
{
                           ...
                           bool isInteractive)
    {
        ...
        {
            if (Time.time - gazeStartTime > 3.0f && UnityEngine.EventSystems.ExecuteEvents.CanHandleEvent<TimedInputHandler>(gazedAt))
            {
                gazeStartTime = -1.0f;
                UnityEngine.EventSystems.ExecuteEvents.Execute(gazedAt, null, (TimedInputHandler handler, UnityEngine.EventSystems.BaseEventData data) => handler.HandleTimeInput());
            }
        }
    }
